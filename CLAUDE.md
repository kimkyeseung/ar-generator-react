...
- URL에 `?mode=log` 추가 시 화면 우측 하단에 콘솔 로그 오버레이 표시
- 기능:
  - log, warn, error, info 레벨별 색상 구분 (회색/노란/빨강/파랑)
  - 타임스탬프 표시 (밀리초 단위)
  - Copy 버튼: 전체 로그 클립보드 복사
  - Clear 버튼: 로그 삭제
  - 최소화 버튼: 접었다 펼치기 (로그 개수 배지 표시)
- 최대 100개 로그 유지
- 모바일에서 개발자 도구 없이 디버깅할 때 유용

### Debug Mode (영상 품질 확인)
- URL에 `?mode=debug` 추가 시 화면 하단에 디버그 패널 표시
- 표시 정보:
  - 📷 카메라 해상도 (실제 적용된 해상도)
  - 🎬 영상 품질 (고화질/중화질/저화질)
