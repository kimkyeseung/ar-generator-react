...
function GetProgram(image) {
  ...
  const kernel = {
    ...
    userCode: function () {
      ...
      if (
        y < FREAK_EXPANSION_FACTOR ||
        y > imageHeight - 1 - FREAK_EXPANSION_FACTOR
      ) {
        this.setOutput(0.0)
        return
      }
      if (
        x < FREAK_EXPANSION_FACTOR ||
        x > imageWidth - 1 - FREAK_EXPANSION_FACTOR
      ) {
        this.setOutput(0.0)
        return
      }
      ...
      const dxx =
        this.getImage1(y, x + 1) +
        this.getImage1(y, x - 1) -
        2 * this.getImage1(y, x)
      const dyy =
        this.getImage1(y + 1, x) +
        this.getImage1(y - 1, x) -
        2 * this.getImage1(y, x)
      const dxy =
        0.25 *
...
export const buildExtremas = (args) => {
  ...
  image2 = engine().runKernel('UpsampleBilinear', {
    image: image2,
    targetImage: image1,
  })
