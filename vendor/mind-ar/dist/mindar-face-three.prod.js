import {
  ...
  WebGLRenderer as w,
  sRGBEncoding as y,
  PerspectiveCamera as g,
  Mesh as M,
  MeshStandardMaterial as x,
  Group as p,
  BufferGeometry as A,
...
class U {
  constructor({
    ...
    ;((this.container = o),
      (this.ui = new E({ uiLoading: e, uiScanning: i, uiError: n })),
      ...
      (this.scene = new m()),
      (this.cssScene = new m()),
      (this.renderer = new w({ antialias: !0, alpha: !0 })),
      (this.cssRenderer = new S({ antialias: !0 })),
  ...
  addFaceMesh() {
    const o = this.controller.createThreeFaceGeometry(C),
      e = new M(o, new x({ color: 16777215 }))
    return (
      (e.visible = !1),
      (e.matrixAutoUpdate = !1),
      this.faceMeshes.push(e),
      e
    )
  }
  addAnchor(o) {
  ...
  _startAR() {
    return new Promise(async (o, e) => {
      const i = this.video
      ;(this.container,
        (this.controller.onUpdate = ({ hasFace: l, estimateResult: f }) => {
          for (let r = 0; r < this.anchors.length; r++)
            this.anchors[r].css
              ? this.anchors[r].group.children.forEach((a) => {
          ...
          if (l) {
            ...
            for (let c = 0; c < this.anchors.length; c++) {
              const u = this.anchors[c].landmarkIndex,
                s = this.controller.getLandmarkMatrix(u)
              ...
              } else this.anchors[c].group.matrix.set(...s)
            }
      ...
      ;((this.camera.fov = n),
        ...
        await this.controller.dummyRun(i),
        this._resize(),
        this.controller.processVideo(i),
        o())
    })
  }
  _resize() {
    ...
    if (!t) return
    let h, d
    const l = t.videoWidth / t.videoHeight,
      f = n.clientWidth / n.clientHeight
    ;(l > f
      ? ((d = n.clientHeight), (h = d * l))
      : ((h = n.clientWidth), (d = h / l)),
      (t.style.top = -(d - n.clientHeight) / 2 + 'px'),
      (t.style.left = -(h - n.clientWidth) / 2 + 'px'),
      (t.style.width = h + 'px'),
      (t.style.height = d + 'px'))
    const r = o.domElement,
      a = e.domElement
    ;((r.style.position = 'absolute'),
      (r.style.top = t.style.top),
      (r.style.left = t.style.left),
      (r.style.width = t.style.width),
      (r.style.height = t.style.height),
      (a.style.position = 'absolute'),
      (a.style.top = t.style.top),
      (a.style.left = t.style.left),
      (a.style.transformOrigin = 'top left'),
      (a.style.transform =
        'scale(' +
        h / parseFloat(a.style.width) +
        ',' +
